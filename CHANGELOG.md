...
- iPhone PWA Standalone 最终验收发现 FAB 与 BottomNav 重叠：BottomNav 会叠加 `safe-area-inset-bottom`，三个 FAB 原先固定为 `bottom: 6rem`。新增共享 `.fab-safe-bottom`，让 FAB 同步叠加安全区并始终保留 16px 间距；Safari 浏览器模式在 inset 为 0 时维持原位置。
